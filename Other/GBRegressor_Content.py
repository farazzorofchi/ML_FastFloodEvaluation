...
all_model_GB = Pipeline([('ohe', OHE_final),
                            ('Predictor_gb', ensemble.GradientBoostingRegressor(loss='ls'))
                            ])

parameters = {'Predictor_gb__learning_rate': [0.1],'Predictor_gb__max_depth':[13,14,15], 'Predictor_gb__min_samples_split':[4], 'Predictor_gb__criterion': ['mse']}

content_model_GB = GridSearchCV(all_model_GB, parameters, cv = 10, verbose=2)
content_model_GB.fit(X_train_cnt, y_train_cnt)

print('best score of test data:', content_model_GB.score(X_test_cnt, y_test_cnt))
print('best parameters of gridsearchcv:', content_model_GB.best_params_)

joblib.dump(content_model_GB.best_estimator_, 'content_model_GB_best_params.pkl')