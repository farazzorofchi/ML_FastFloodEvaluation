...
XGBoost_model = Pipeline([('ohe', OHE_final),
                            ('Predictor_xgboost', xgb.XGBRegressor(objective="reg:squarederror", random_state=42, nthred=3))
                            ])

parameters = {'Predictor_xgboost__booster':['gbtree'], 'Predictor_xgboost__max_depth':[12,13,14,15,16], 'Predictor_xgboost__lambda':[0.01,0.1,1,10,100] }

XGBoost_model_grid = GridSearchCV(XGBoost_model, parameters, cv = 10, verbose=2, n_jobs=2)
...
joblib.dump(XGBoost_model_grid.best_estimator_, 'XGBoost_model_grid_best_params.pkl')
